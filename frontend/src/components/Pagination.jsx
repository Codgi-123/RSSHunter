  ...
  return (
    <nav className="pagination-bar" aria-label="分页">
      ...
      <div className="pagination-controls">
        <select className="pagination-select" aria-label="每页条数" value={pageSize} onChange={(event) => onPageSizeChange(Number(event.target.value))}>
          {pageSizes.map((size) => <option key={size} value={size}>{size} 条/页</option>)}
        </select>
        <button type="button" className="page-button icon-page" aria-label="上一页" onClick={() => go(page - 1)} disabled={page <= 1}><ChevronLeft size={16} /></button>
        {pages.map((item) => <button key={item} type="button" className={`page-button ${item === page ? 'active' : ''}`} aria-label={`第 ${item} 页`} aria-current={item === page ? 'page' : undefined} onClick={() => go(item)}>{item}</button>)}
        <button type="button" className="page-button icon-page" aria-label="下一页" onClick={() => go(page + 1)} disabled={page >= totalPages}><ChevronRight size={16} /></button>
