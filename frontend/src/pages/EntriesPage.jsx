  ...
  return (
    <>
      ...
      <div className="page-block">
        ...
        <div className="filter-bar entries-filter-bar"><ClearableInput className="filter-field tabs-search" value={filters.keyword} onChange={(value) => update('keyword', value)} placeholder="搜索标题和摘要" label="动态搜索" icon={<Search size={15} />} /><DateRangeFilter start={filters.start} end={filters.end} onChange={update} /><ClearableSelect multiple value={filters.vendor} onChange={(value) => update('vendor', value)} label="厂商"><option value="">厂商</option>{vendors.map((item) => <option key={item}>{item}</option>)}</ClearableSelect><ClearableSelect multiple value={filters.product} onChange={(value) => update('product', value)} label="产品"><option value="">产品</option>{products.map((item) => <option key={item}>{item}</option>)}</ClearableSelect><ClearableSelect multiple value={filters.db_type} onChange={(value) => update('db_type', value)} label="数据库类型"><option value="">数据库类型</option>{dbTypes.map((item) => <option key={item}>{item}</option>)}</ClearableSelect><button type="button" onClick={resetFilters}>重置筛选</button></div>
