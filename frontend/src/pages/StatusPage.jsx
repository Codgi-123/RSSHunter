...
export default function StatusPage({ feeds, logs, reloadFeeds, reloadLogs }) {
  ...
  return (
    <>
      ...
      <section className="panel"><div className="panel-header"><h2>订阅源状态</h2><button onClick={reloadFeeds}><RefreshCw size={16} />刷新状态</button></div><div className="table-wrap"><table className="data-table"><thead><tr><th>订阅源</th><th>厂商</th><th>产品</th><th>状态</th><th>最近抓取时间</th><th>错误信息</th><th>操作</th></tr></thead><tbody>{feeds.map((feed) => <tr key={feed.id}><td>{feed.name}</td><td>{feed.vendor}</td><td>{feed.product}</td><td><StatusPill status={feed.status} enabled={feed.enabled} /></td><td>{formatDateTime(feed.last_fetched_at)}</td><td className="summary-cell">{feed.last_error || '最近抓取成功'}</td><td><button className="primary-mini" onClick={() => refresh(feed)}>重试</button></td></tr>)}</tbody></table></div></section>
      <section className="panel"><div className="panel-header"><h2>抓取记录</h2><button onClick={reloadLogs}><RefreshCw size={16} />刷新记录</button></div><div className="table-wrap"><table className="data-table"><thead><tr><th>抓取时间</th><th>订阅源</th><th>抓取结果</th><th>新增条目数</th><th>总条目数</th><th>错误信息</th></tr></thead><tbody>{logs.map((log) => <tr key={log.id}><td>{formatDateTime(log.started_at)}</td><td>{log.feed_name}</td><td><StatusPill status={log.result} /></td><td>{log.new_entries}</td><td>{log.total_entries}</td><td className="summary-cell">{log.error_message || '-'}</td></tr>)}{!logs.length && <tr><td colSpan="6" className="empty-cell">暂无抓取记录</td></tr>}</tbody></table></div></section>
    </>
  );
}
